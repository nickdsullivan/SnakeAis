...
class Strategy:
        ...
        for x in range(board.width):
            for y in range(board.height):
                if board.get_square((x,y)) == board.TILES["APPLE"]:
                    goal = (x,y)
                    last_node = self.dfs(board,head,goal,True)
                    if last_node == None:
                        return None
    ...
    def dfs(self,board,head,goal,first_flag):
        path_node = None
        visited = []
        queue = []
        ...
        while len(queue) != 0:
            ...
            if current.location ==  goal:
                if first_flag:
                    if self.dfs(board,current, self.tail.location,False) != None:
                        return current
                else:
                    return current
